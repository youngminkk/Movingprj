        ...
        <div class="container ml-5">
            <div class="container">
                <div class="container-fluid p-0">
						...
						<!--영화 검색 -->
						<form action="/admin/movie/delete" method="get" id="searchForm">
						...
						<form action="/admin/movie/delete" method="delete" id="deleteForm">						
                            ...
                            <thead class="table-dar">
                                <tr>
                                	<th><input type="checkbox" id="selectAllCheckbox"></th>                               
                                                ...
                                                <th>국가분류</th>
                                                <th>상영여부</th>
                                   	<th><button type="button" class="btn btn-danger p-1" id="deleteSelectedBtn" disabled>영화 삭제</button></th>
                                </tr>
                            </thead>
                            <tbody>
									<c:forEach items="${list}" var="mv">
										<tr class="movie-row">
											<td><input type="checkbox" class="delete-checkbox" data-mno="${mv.mno}"></td>
											...
											<td><button type="button" class="btn btn-danger delete-btn" data-mno="${mv.mno}">삭제</button></td>
										</tr>
									</c:forEach>
									<c:if test="${empty list}">
                                    <tr>
                                        <td colspan="7">일치하는 영화가 없습니다</td>
                                    </tr>
                                </c:if>
                            </tbody>
                        </table>
...
$(document).ready(function() {
    ...
    // 선택된 영화 삭제 버튼 클릭 이벤트 처리
    $('#deleteSelectedBtn').on('click', function() {
    	 if (confirm('선택한 영화를 삭제하시겠습니까?')) {  
        var mnos = $('.delete-checkbox:checked').map(function() {
            return $(this).data('mno');
        }).get();
    ...
    // 회원 삭제 
    function deleteMovies(mnos) {
    	console.log("영화 삭제 요청을 보냅니다. 영화 번호: ", mnos);  
        $.ajax({
            url: '/admin/movie/delete',
            type: 'DELETE',
            data: JSON.stringify(mnos),
            contentType: 'application/json',
            success: function(result) {
                if (result) {
                    alert("성공적으로 영화를 삭제하였습니다.");
                    location.reload();
                } else {
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                console.log("영화 삭제 요청이 실패했습니다. 오류: ", textStatus, errorThrown); 
                alert("영화 삭제에 실패하였습니다.");
            }
        });
    }
